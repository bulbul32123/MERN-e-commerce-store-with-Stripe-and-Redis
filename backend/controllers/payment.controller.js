...
export const createCheckoutSession = async (req, res) => {
    try {
        ...
        console.log('Error Occur in processing ', error);
...
export const checkoutSuccess = async (req, res) => {
    try {
        ...
        if (session.payment_status === "paid") {
            ...
            await newOrder.save()
            res.status(200).json({ success: true, message: "Payment Successful, order created, and coupon deactivated id used.", orderId: newOrder._id })
        }
    } catch (error) {
        console.log('Error Occur in processing successful checkout ', error);
